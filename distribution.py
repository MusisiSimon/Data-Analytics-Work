...
NEW:
BUGFIX
Fixed bug on pd.to_datetime that was reading months first (US date format) instead of day first (UK format)
"""
...
FOOD_RATION_DAILY = ((0.4)+(.08) + (.03) + (.005) + (.05))
FOOD_RATION_MONTHLY = ((0.4*31)+(.08*31) + (.03*31) + (.005*31) + (.05)*31)
RECENT_DATE = datetime.date.today()
MONTHLY_CASH_RATION = 31000
CASH_RATION_MONTHLY = 31000
...
def concatenate(*args, **kwargs):
    ...
    for key, frame in enumerate(data_frames):
        ...
        frame.loc[:,"UpdateDate"] = pd.to_datetime(frame.UpdateDate, dayfirst=True)
        frame["settlement"] = settlement["name"]
        frame["FDP"] = settlement["fdp"].split("_")[0].title()
        frame["assistance"] = settlement["benefit"]
...
def add_table(df, doc, **kwargs):
    """
    Function adds table to word document
    """
...
def pivot_table(df, **kwargsl):
    ...
    df.loc[:, "UpdateDate"] = pd.to_datetime(df["UpdateDate"], dayfirst=True)
...
def pt_food(df, doc, **kwargs):
    ...
    df_distr.loc[:,"Total"] = df_distr[["CSB", "Pulses", "Salt", "Vegetable_Oil", "Cereal"]].sum(axis=1)
...
def litigation(df_litigation, doc, *args, **kwargs):
    ...
    df_litigation.loc[:, "Date Time"] = pd.to_datetime(df_litigation["Date Time"], dayfirst=True)
    ...
    for month in months.index:
        ...
        df_lit_pivot.loc["Total"] = df_lit_pivot.sum()
...
def daily(): 
    ...
    for key, frame in frames.items():
        ...
        frame["UpdateDate"] = pd.to_datetime(frame["UpdateDate"], dayfirst=True)
...
def cycle():
    ...
    while True:
        ...
        try:
            cycle = int(cycle)
            if cycle > 12:
                raise ValueError("Cycle should be in the range 1-12")   
            break
        except ValueError as verror:
            print("Invalid input! " + repr(verror))


    print("Processing manifests ...")
    frames = concatenate(export=False)
    settlement = frames.settlement[0]
    document_heading = {"first_heading": "Report for Distribution Cycle {0:d}".format(cycle)}
    doc = create_document(document_heading)
    doc.add_heading(f"Settlement: {settlement}", level=0)
    frames.loc[:, "cycle"] = pd.to_numeric(frames.cycle)

    lowest_cycle = frames.cycle.min()
    highest_cycle = frames.cycle.max()
    cycle_heading = "Total Number of beneficiaries eligible "
    if lowest_cycle == highest_cycle:
        cycle_heading = cycle_heading + f" in cycle {lowest_cycle}"
    else:
        cycle_heading = cycle_heading + f" from {lowest_cycle} to {highest_cycle} "

    doc.add_heading(cycle_heading, level=1)
    #Show trends of populations on food and cash
    pivot = frames.pivot_table(aggfunc=np.sum, index="assistance", \
            columns="cycle", values="ProcessingGroupSize")
    pivot.T.plot(kind="bar")
    ...
    ax.grid(which='minor', alpha=0.4)
    ax.minorticks_on()
    #save image to stream variable instead of file on disk
    memplot = io.BytesIO()
    plt.savefig(memplot)
    doc.add_picture(memplot)
    pivot.loc["Total"] = pivot.sum()
    pivot.insert(0, "Assistance", pivot.index)
    pivot = sanitise_table_header(pivot)
    add_table(pivot, doc)
    #d = {"first_heading": calendar.month_name[cycle] + " Distribution Report"}
    doc.add_heading("Distribution Trends", level=1)
    df_collected = frames[frames.Status == "Collected"]
    df_cash_collected = df_collected[df_collected.assistance == "Cash"]
    ...
    #df_cash_collected.plot();
    #plt.s
    df_cash_collected_pv = sanitise_table_header(df_cash_collected_pv)
    add_table(df_cash_collected_pv, doc)
    df_food_collected = frames[frames.assistance=="Food"]

    frames_hh_benefit_pivot = frames.pivot_table(aggfunc=[len, np.sum], columns="cycle", index="assistance", values="ProcessingGroupSize")
    frames_popn_benefit_pivot = frames.pivot_table(aggfunc=[len, np.sum], columns="cycle", index=["assistance", "Status"], values="ProcessingGroupSize")
    ...
    frames_popn_benefit_pivot.insert(0, "Index", frames_popn_benefit_pivot.index)
    ...
    df_food = df[df.assistance == "Food"]
    df_cash = df[df.assistance == "Cash"]
...
def span():
    """
    Function retrieves rows between two dates
    """
    ...
    df.loc[:, "UpdateDate"] = pd.to_datetime(df.UpdateDate, dayfirst=True)
    df.set_index(["UpdateDate"], inplace=True)
    df_range = df.loc[start_date:end_date]
    df_range_collected = df_range[df_range.Status == "Collected"]
    pt = df_range_collected.pivot_table(index="ProcessingGroupSize", aggfunc=len\
            , columns="assistance", values="Status")
    pt.loc[:, "Cash_Population"] = pt.index * pt.Cash
    pt.loc[:, "Food_Population"] = pt.index * pt.Food
    pt.loc[:, "Cash_Amount"] = pt.Cash_Population * CASH_RATION_MONTHLY
    pt.loc[:, "Food Amount"] = pt.Food_Population * FOOD_RATION_MONTHLY
    pt.loc["Total"] = pt.sum()
    pt.index.rename("Family Size", inplace=True)
    pt.insert(0, "Family Size", value=pt.index)
    export_excel(pt)
